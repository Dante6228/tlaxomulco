...
document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        try {
            const response = await fetch("php/alumnos/obtener_matricula2.php", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
                body: `matricula=${encodeURIComponent(matricula.value)}&excluir=${encodeURIComponent(matriculaExistente.value)}`,
            });
            ...
            if (data.trim() === "ya existe") {
                ...
                matricula.classList.add("input-error");
                valid = false;
            } else if (data.trim() === "es valida") {
                errorMat2.innerText = ""; // Limpiar errores
                errorMat2.classList.remove("error-message");
                matricula.classList.remove("input-error");
            } else {
                console.error("Error inesperado en el servidor:", data);
                valid = false;
            }
        } catch (error) {
