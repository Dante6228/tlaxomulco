...
async function mostrarAlumnos() {
    ...
    const promocionId = document.getElementById("promocion").value;

    const tbody = document.querySelector("tbody");
    tbody.innerHTML = "";

    if (nivelEducativoId && gradoId && cicloId && promocionId) {
        ...
        try {
            const response = await fetch("../php/consulta/promocion.php", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
        ...
        Swal.fire({
            title: 'Faltan datos',
            text: 'Por favor, selecciona un nivel educativo, grado, ciclo escolar y promoción.',
            icon: 'warning',
...
async function generarPDF() {
    ...
    try {
        ...
        a.download = `Listado_de_alumnos_por_promocion_de_alumno_${cicloTexto}.pdf`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);

        window.location.href = '/tlaxomulco/consulta/promocion.php?mensaje=pdf';
...
async function generarExcel() {
    ...
    try {
        ...
        a.style.display = 'none';
        a.href = url;
        a.download = 'Listado_de_Alumnos_Por_Promocion.xlsx';
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);

        window.location.href = '/tlaxomulco/consulta/promocion.php?mensaje=excel';
