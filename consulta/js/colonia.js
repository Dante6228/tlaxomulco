...
async function mostrarAlumnos() {
    ...
    const coloniaId = document.getElementById("colonia").value;

    const tbody = document.querySelector("tbody");
    tbody.innerHTML = "";

    if (nivelEducativoId && gradoId && cicloId && coloniaId) {
        ...
        try {
            const response = await fetch("../php/consulta/colonia.php", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
        ...
        alert("Por favor, selecciona un nivel educativo, grado, ciclo escolar y colonia.");
    }
}
...
async function generarPDF() {
    ...
    try {
        ...
        a.download = `Listado_de_alumnos_por_colonia_de_alumno_${cicloTexto}.pdf`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);

        window.location.href = '/tlaxomulco/consulta/colonia.php?mensaje=pdf';
