...
async function mostrarAlumnos() {
    ...
    if (nivelEducativoId && gradoId && cicloId && generoId) {
        ...
        try {
            const response = await fetch("../php/consulta/genero.php", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
        ...
        Swal.fire({
            title: 'Faltan datos',
            text: 'Por favor, selecciona un nivel educativo, grado, ciclo escolar y genero.',
            icon: 'warning',
...
async function actualizarAlumno(event) {
    ...
    window.location.href = `../actualizar_alumno.php?id=${alumnoId}&from=6`;
}
...
async function generarPDF() {
    ...
    try {
        ...
        a.download = `Listado_de_alumnos_por_genero_de_alumno_${cicloTexto}.pdf`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);

        window.location.href = '/tlaxomulco/consulta/genero.php?mensaje=pdf';
...
async function generarExcel() {
    ...
    try {
        ...
        a.style.display = 'none';
        a.href = url;
        a.download = 'Listado_de_Alumnos_Por_Genero.xlsx';
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);

        window.location.href = '/tlaxomulco/consulta/genero.php?mensaje=excel';
